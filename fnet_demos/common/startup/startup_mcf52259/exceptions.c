...
#define VECTORDISPLAY(MESSAGE)                    fnet_mcf_nop();
#define VECTORDISPLAY2(MESSAGE,MESSAGE2)          fnet_mcf_nop();
#define VECTORDISPLAY3(MESSAGE,MESSAGE2,MESSAGE3) fnet_mcf_nop();


extern unsigned long _SP_INIT[];
...
extern void _startup(void);
    ...
    #define STARTUP __program_start
        ...
        const vectorTableEntryType ram_vector[256] @ "INTVECRAM";// =
    #endif          
    #if FNET_CFG_COMP_CW
        ...
        __declspec(vectors_ram) vectorTableEntryType ram_vector[256 + 6];// = 
    #endif  
#endif
...
{   /* Interrupt vector table */
  (vectorTableEntryType)__SP_AFTER_RESET,  /*   0 (0x000) Initial supervisor SP      */
   ...
   asm_exception_handler,           /*  85 (0x154) Device-specific interrupts */
    ...
    FNET_ISR_HANDLER,               /****  86 (0x158) Timer 3 capture/reference event ****/
#else
...
#if FAPP_CFG_PREINSTALL_INTERRUPTS
    FNET_ISR_HANDLER,               /****  91 (0x16C) FEC Receive frame interrupt ****/
#else
...
void initialize_exceptions(void)
{
    ...
    */
    ...
    fnet_mcf_vbr_wr((unsigned long)__VECTOR_RAM);
}
