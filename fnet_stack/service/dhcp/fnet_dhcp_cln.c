...
#define FNET_DHCP_CLN_STATE_SELECTING_SEND_TIMEOUT      (FNET_CFG_DHCP_CLN_RESPONSE_TIMEOUT*1000U)     /*(ms) timeout for OFFER => INIT.*/
...
*     Function Prototypes
*************************************************************************/
static fnet_uint8_t *fnet_dhcp_cln_next_option(fnet_dhcp_cln_message_t *message);
static void fnet_dhcp_cln_parse_options(fnet_dhcp_cln_message_t *message, struct fnet_dhcp_cln_options_in *options);
...
************************************************************************/
static void fnet_dhcp_cln_print_state( fnet_dhcp_cln_if_t *dhcp )
{
    FNET_DEBUG_DHCP_CLN("\nDHCP: State ");
    switch(dhcp->state)
    {
...
************************************************************************/
static void fnet_dhcp_cln_print_options( struct fnet_dhcp_cln_options_in *options )
{
...
#define fnet_dhcp_cln_print_state(x) do{}while(0)
#define fnet_dhcp_cln_print_options(x) do{}while(0)
...
static void fnet_dhcp_cln_parse_options( fnet_dhcp_cln_message_t *message, struct fnet_dhcp_cln_options_in *options )
{
    ...
    while((current_option = fnet_dhcp_cln_next_option(message)) != 0)
    {
...
static void fnet_dhcp_cln_send_message( fnet_dhcp_cln_if_t *dhcp )
{
    ...
    switch(dhcp->state)
    {
        ...
        case FNET_DHCP_CLN_STATE_REQUESTING: /*=>Requesting*/
            ...
            option_position = fnet_dhcp_add_option(option_position, ((message->header.options + sizeof(message->header.options)) - option_position), FNET_DHCP_OPTION_REQ_ADDRESS, FNET_DHCP_OPTION_REQ_ADDRESS_LENGTH,  &dhcp->offered_options.public_options.ip_address);
            if(option_position == FNET_NULL)
            {
                goto EXIT;
            }

            option_position = fnet_dhcp_add_option(option_position, ((message->header.options + sizeof(message->header.options)) - option_position), FNET_DHCP_OPTION_SERVER_ID, FNET_DHCP_OPTION_SERVER_ID_LENGTH,  &dhcp->offered_options.public_options.dhcp_server);
            if(option_position == FNET_NULL)
            {
                goto EXIT;
            }
        ...
        case FNET_DHCP_CLN_STATE_REBOOTING: /*=>Requesting*/
            ip_address.s_addr = INADDR_BROADCAST;
            option_position = fnet_dhcp_add_option(option_position, ((message->header.options + sizeof(message->header.options)) - option_position), FNET_DHCP_OPTION_REQ_ADDRESS, FNET_DHCP_OPTION_REQ_ADDRESS_LENGTH, &dhcp->in_params.requested_ip_address);
            if(option_position == FNET_NULL)
            {
                goto EXIT;
            }
    ...
    option_position = fnet_dhcp_add_option(option_position, ((message->header.options + sizeof(message->header.options)) - option_position), FNET_DHCP_OPTION_MSG_TYPE, FNET_DHCP_OPTION_MSG_TYPE_LENGTH, &message_type);
    if(option_position == FNET_NULL)
    {
        goto EXIT;
    }
    ...
    if((message_type == FNET_DHCP_OPTION_MSG_TYPE_REQUEST) || (message_type == FNET_DHCP_OPTION_MSG_TYPE_DISCOVER))
    {
        ...
        {
            option_position = fnet_dhcp_add_option(option_position, ((message->header.options + sizeof(message->header.options)) - option_position), FNET_DHCP_OPTION_LEASE, FNET_DHCP_OPTION_LEASE_LENGTH, &dhcp->in_params.requested_lease_time);
            if(option_position == FNET_NULL)
            {
                goto EXIT;
            }
        ...
        option_position = fnet_dhcp_add_option(option_position, ((message->header.options + sizeof(message->header.options)) - option_position), FNET_DHCP_OPTION_PARAMETER_REQ_LIST, sizeof(fnet_dhcp_cln_parameter_request_list), &fnet_dhcp_cln_parameter_request_list);
        if(option_position == FNET_NULL)
        {
            goto EXIT;
        }
    ...
    option_position = fnet_dhcp_add_option(option_position, ((message->header.options + sizeof(message->header.options)) - option_position), FNET_DHCP_OPTION_MESSAGE_SIZE, FNET_DHCP_OPTION_MESSAGE_SIZE_LENGTH, &max_message_size);
    if(option_position == FNET_NULL)
    {
        goto EXIT;
    }
    ...
    option_position = fnet_dhcp_add_option(option_position, ((message->header.options + sizeof(message->header.options)) - option_position), FNET_DHCP_OPTION_CLIENT_ID, FNET_DHCP_OPTION_CLIENT_ID_LENGTH, &client_id);
    if(option_position == FNET_NULL)
    {
        goto EXIT;
    }
...
static void fnet_dhcp_cln_set_state( fnet_dhcp_cln_if_t *dhcp, fnet_dhcp_cln_state_t state )
{
    ...
    fnet_dhcp_cln_print_state(dhcp);
    ...
    switch(state)
    {
        ...
        case FNET_DHCP_CLN_STATE_INIT:
            ...
            break;
        case FNET_DHCP_CLN_STATE_SELECTING:
            fnet_dhcp_cln_send_message(dhcp);                          /* Send DISCOVER */
...
#if !FNET_CFG_DHCP_CLN_BOOTP
        case FNET_DHCP_CLN_STATE_REQUESTING:
            fnet_dhcp_cln_send_message(dhcp); /* Send REQUEST.*/
        ...
        case FNET_DHCP_CLN_STATE_DISABLED:
            ...
            {
                fnet_dhcp_cln_send_message(dhcp);                           /* Send RELEASE */
            }
...
static void fnet_dhcp_cln_poll( void *fnet_dhcp_cln_if_p )
{
    ...
    switch(dhcp->state)
    {
        ...
        case FNET_DHCP_CLN_STATE_SELECTING:
            /* Waiting for OFFER */
            res = fnet_dhcp_cln_receive_message(dhcp, &options);
            ...
            else if(res > 0)
            {
                ...
                * to ensure that the address is not already in use.*/
                fnet_dhcp_cln_print_options(&options);
                /* Apply parameters. */
                dhcp->current_options = options;

                fnet_dhcp_cln_apply_params(dhcp);
            }
#else /* DHCP */
        ...
        case FNET_DHCP_CLN_STATE_REQUESTING:    /*---- REQUESTING ----------------------------*/
            ...
            if(fnet_timer_get_interval(dhcp->lease_obtained_time, fnet_timer_get_ticks()) < dhcp->state_timeout)
            {
                ...
                if(res == FNET_DHCP_CLN_IS_TIMEOUT)
                {
                    fnet_dhcp_cln_send_message(dhcp); /* Resend REQUEST.*/
                }
                else if(res > 0)
                {
...
fnet_dhcp_cln_desc_t fnet_dhcp_cln_init(struct fnet_dhcp_cln_params *params )
{
    ...
    dhcp_if->service_descriptor = fnet_service_register(fnet_dhcp_cln_poll, (void *) dhcp_if);
...
/************************************************************************
* DESCRIPTION: Looks for a DHCP Client assigned to the specified network interface.
************************************************************************/
