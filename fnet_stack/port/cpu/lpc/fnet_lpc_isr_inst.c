...
fnet_return_t fnet_cpu_isr_install(fnet_uint32_t vector_number, fnet_uint32_t priority)
{
    ...
    if(vector_number > 16)
    {
        ...
        if(divider < 5u)
        {
            ...
            FNET_LPC_NVIC_ISER(divider) |= (fnet_uint32_t)(1u << (irq_number % 32u)); /* Set-enable.*/
            /* Set priority.*/
            FNET_LPC_NVIC_IP(irq_number) = (fnet_uint8_t)(FNET_CFG_CPU_VECTOR_PRIORITY_MAX - priority) << 4;
        }
        result = FNET_OK;
    }
    else
