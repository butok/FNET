...
/*
 * If the application provides its own implementation of vApplicationIRQHandler(),
 ...
 * vApplicationFPUSafeIRQHandler() then this weak implementation of
 * vApplicationIRQHandler() will save the FPU registers before calling it.
 *
 ...
 *
 * Therefore, if the application writer wants FPU registers to be saved on
 * interrupt entry, their IRQ handler must be called by
 * vApplicationFPUSafeIRQHandler(), and if the application writer does not want
 * FPU registers to be saved on interrupt entry their IRQ handler must be
 * called directly by vApplicationIRQHandler().
 */

#pragma weak vApplicationFPUSafeIRQHandler
void vApplicationFPUSafeIRQHandler(uint32_t ulICCIAR)
{
	(void)ulICCIAR;
...
#pragma weak vApplicationIRQHandler
void vApplicationIRQHandler(uint32_t ulICCIAR)
{
	uint32_t ulInitialFPSCR = 0;
	__asm(
	...
	vApplicationFPUSafeIRQHandler(ulICCIAR);
