...
A_STATUS WIFISHIELD_Init(void)
{
    ...
    GPIO_PortInit(GPIO, BOARD_INITGT202SHIELD_PWRON_PORT);
    ...
    GPIO_PortInit(GPIO, BOARD_INITGT202SHIELD_IRQ_PORT);
...
//#endif
    ...
    spi_config.spi.clk_hz = CLOCK_GetFreq(WIFISHIELD_SPI_CLK);
...
uint32_t g_tmp_pintr = 0;
uint32_t g_tmp_status = 0;

/* ISR callback for WLAN_IRQ */
static void PINT_callback(pint_pin_int_t pintr, uint32_t pmatch_status)
{
    g_tmp_pintr = pintr;
    g_tmp_status = pmatch_status;
